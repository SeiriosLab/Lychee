...
class DataAugmentationOnDetection:
    ...
    def add_pepper_noise(self, img):
        ...
        img[noise[:, :, :]>alpha]=0
        return img
    def add_poisson_noise(self, img, lam=1.0):
        # Generate Poisson noise
        noise = torch.poisson(lam * torch.ones_like(img)) / lam
        # Add noise to image
        noisy_img = img + (noise - 1)  # Center around 0
        # Clip to maintain valid pixel range
        noisy_img = noisy_img.clamp(min=0.0, max=1.0)
        return noisy_img
...
def runAugumentation(image_path, label_path, save_path):
    ...
    for image_name in image_list:
        ...
        # Poisson noise
        t_img, t_boxes = DAD.add_poisson_noise(img.clone()), boxes
        save_Yolo(to_image(t_img), boxes, save_path, prefix="poisson_", image_name=image_name)
...
if __name__ == '__main__':
    # 1. 定义命令行参数解析器
    parser = argparse.ArgumentParser(description='数据增强脚本：输入图像路径、标签路径和保存路径')
    parser.add_argument('--image_path', type=str, required=True, help='输入图像文件夹路径')
    parser.add_argument('--label_path', type=str, required=True, help='输入标签文件夹路径')
    parser.add_argument('--save_path', type=str, required=True, help='增强结果保存路径')
    # 2. 解析参数
    args = parser.parse_args()
    # 3. 调用增强函数
    runAugumentation(args.image_path, args.label_path, args.save_path)
