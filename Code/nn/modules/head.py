...
class RTDETR_KANLinerHead_Decoder(nn.Module):
    """
    ...
    def __init__(
        ...
        self.dec_score_head = nn.ModuleList([
            GroupKANLinear(hd, nc, num_groups=1, act_mode="gelu") for _ in range(ndl)
        ])
        self.dec_bbox_head = nn.ModuleList([MLP(hd, hd, 4, num_layers=3) for _ in range(ndl)])
        # self.dec_bbox_head = nn.ModuleList([
        #     GroupKANLinear(hd, 4, num_groups=8, act_mode="gelu") for _ in range(ndl)
        # ])

        self._kan_reset_parameters()
    ...
    def _kan_reset_parameters(self):
        ...
        for cls_, reg_ in zip(self.dec_score_head, self.dec_bbox_head):
            if hasattr(cls_, "linear"):
                constant_(cls_.linear.bias, bias_cls)
            if hasattr(reg_, "linear"):
                constant_(reg_.linear.weight, 0.0)
                constant_(reg_.linear.bias, 0.0)
