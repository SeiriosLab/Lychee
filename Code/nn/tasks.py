    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB}:
                m.legacy = legacy
        elif m in {RTDETRDecoder, RTDETR_KANLinerHead_Decoder}:  # special case
            ...
            c2 = 4*ch[f]
        elif m is GhostResNetBlock:
            c1 = ch[f]
            c2 = args[1]
            print(f"[------------task] GhostResNetBlock: c1={c1}, c2={c2}, ch={ch}")
        elif m is ECA:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
